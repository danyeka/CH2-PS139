...
# app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:password@/rempahustle?unix_socket=/cloudsql/rempahustle:asia-southeast2:rempahustle'
db = SQLAlchemy(app)
...
class Resep(db.Model):
    ...
    nama_resep = db.Column(db.String(255), nullable=False)
    image = db.Column(db.String(255), nullable=False)

def get_resep_terkait(resep_terkait):
    # Split resep_terkait menjadi list nama resep
    list_nama_resep = resep_terkait.split(',')

    # Ambil informasi resep berdasarkan nama resep dari list
    resep_info = []
    for nama_resep in list_nama_resep:
        resep_data = Resep.query.filter_by(nama_resep=nama_resep.strip()).first()
        if resep_data:
            resep_info.append({
                "id_resep": resep_data.id_resep,
                "nama_resep": resep_data.nama_resep,
                "image": resep_data.image
            })
...
# Load your .h5 model
model = load_model('Model_Rempah_Final.h5', custom_objects={'req': req})
        ...
        if rempah_data:
            result = {
                ...
                "data": {
                    ...
                    "resep_terkait": get_resep_terkait(rempah_data.resep_terkait)
                }
            }
        else:
